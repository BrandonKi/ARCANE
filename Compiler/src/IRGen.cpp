...
void IRGen::gen_for(ForStmnt* for_stmnt, arcvm::Block* ir_gen) {
    ...
    auto op1 = gen_var_load(id, ind_var_type, loop_cmp);
    ...
    auto ind_val = gen_var_load(id, ind_var_type, loop_inc);
...
arcvm::IRValue IRGen::gen_expr(Expr* expr, arcvm::BasicBlock* ir_gen) {
    ...
    switch(expr->type) {
        ...
        case EXPR_STRING_LIT:
            ...
            break;
        case EXPR_ID:
            return gen_var_load(expr->id.val, type, ir_gen);
            break;
        case EXPR_FN_CALL:
...
arcvm::IRValue IRGen::gen_var_load(std::string* id, arcvm::Type type, arcvm::BasicBlock* ir_gen) {
    PROFILE();
...
arcvm::IRValue IRGen::gen_lrvalue_expr(Expr* expr, arcvm::BasicBlock* ir_gen) {
    ...
    auto lhs_val = gen_var_load(lhs_id_ptr, type, ir_gen); // TODO make sure this is the right type to pass
